...
def is_valid(url):
    try:
        ...
        not re.match(const.DEFAULT_INVALID_RE, parsed_url.path.lower()) and \
            ...
            re.match(const.WIKI_TRAP_RE, parse.path) and \
            re.match(const.EDIT_FILE_TRAP_RE, url) and \
            re.match(const.MEDIA_FILE_TRAP_RE, url) and \
            re.match(const.REPEATED_DIR_TRAP_RE, parsed.path)
